...
function getCartTotalAmount() {
  ...
  SHOPPING_CART.forEach( value =>{
    PRODUCTS.forEach(pro => {
      if(value.id === pro.id){
        result += (pro.price * value.quantity);
      }
    });
  });
  return result;
...
function addProductToCart(productId) {
  ...
  if( productId >0 && productId <=5){
    let product = SHOPPING_CART.find(element => element.id == productId)
    if(product){
      product.quantity++
    }else{
...
function removeProductFromCart(productId) {
  ...
  if(productId >0 && productId <=5 ){
    let proIndex;
    let product = SHOPPING_CART.find( (value ,index)=> {
    ...
    if(product && product.quantity >=2){
      ...
      SHOPPING_CART.splice(proIndex,1);
    }
